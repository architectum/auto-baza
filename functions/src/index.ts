 ...
 */
export const checkUnresolvedProblems = onSchedule({
  schedule: '0 9 * * *',
  ...
  try {
    ...
    for (const userDoc of activeUsers) {
      ...
      const thresholdDate = addDays(new Date(), -remindDays);
      ...
      if (carsSnap.empty) continue;

      let totalUnresolvedProblems = 0;
      const carsWithProblems: { make: string; model: string; plate: string; count: number }[] = [];
