...
interface Props {
  ...
  isNew: boolean;
  onSave: () => void;
  onAICarData: (data: Partial<Car>) => void;
}

export function CarForm({ car, setCar, isNew, onSave, onAICarData }: Props) {
  const [hasYear, setHasYear] = useState(!!car.year);

  const handleClientVoice = (data: any) => {
    setCar({ ...car, clientName: data.clientName || car.clientName, clientPhone: data.clientPhone || car.clientPhone });
  };
    ...
    <div className="rounded-2xl p-5 border animate-fade-in-up" style={{ background: 'var(--t-surface-card)', borderColor: 'var(--t-border-default)' }}>
      ...
      <div className="flex items-center justify-between gap-3 p-3.5 rounded-xl mb-5" style={{ background: 'var(--t-accent-primary-muted)', border: '1px solid var(--t-border-accent)' }}>
        ...
        <div className="flex items-center gap-3">
          <PhotoAssistant onDataExtracted={onAICarData} />
          <VoiceAssistant context="car" onDataExtracted={onAICarData} className="!flex-row" />
        </div>
      </div>
      ...
      <div className="pt-5 mt-5 border-t" style={{ borderColor: 'var(--t-border-default)' }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <VoiceAssistant context="client" onDataExtracted={handleClientVoice} size="sm" className="!flex-row" />
        </div>
