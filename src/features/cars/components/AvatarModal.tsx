...
import { X, Sparkles, Loader2, Check, Download, ChevronLeft, ChevronRight, Trash2 } from '@shared/icons/Icons';
...
export function AvatarModal({ 
  ...
  useEffect(() => {
    const fetchAvatars = async () => {
      setLoading(true);
      const res = await listFolderFiles(`${userId}/${carId}/avatar/gen`);
  ...
  const handleDelete = async () => {
    ...
    if (current.url === carPhotoUrl) {
      toast.warning('Це основне фото автомобіля. Його не можна видалити зі списку аватарів.');
      return;
    }
    
    const isConfirmed = await confirm('Видалити цей аватар?');
    if (!isConfirmed) return;
    
    try {
      const res = await deleteFromStorage(current.path);
